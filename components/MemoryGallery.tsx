...
import { ArrowLeft, Clock, Calendar, Activity, BookOpen, BrainCircuit, CheckCircle, RefreshCw } from 'lucide-react';
...
export const MemoryGallery: React.FC<MemoryGalleryProps> = ({ memories, onClose }) => {
  return (
    <div className="absolute inset-0 z-50 bg-black flex flex-col">
      <div className="p-6 flex items-center border-b border-white/10 bg-black/50 backdrop-blur-md z-10">
        <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-colors mr-4">
          <ArrowLeft size={24} className="text-white" />
        </button>
        <h1 className="text-xl font-serif italic text-white">知识宇宙 (Cosmos of Knowledge)</h1>
      </div>
      ...
      <div className="flex-1 overflow-x-auto p-8 flex items-center space-x-8 scrollbar-hide">
        ...
        {memories.map((mem) => (
          <div key={mem.id} className="min-w-[400px] w-[400px] h-[650px] bg-zinc-900/80 border border-white/10 rounded-2xl flex flex-col relative group hover:border-blue-400/50 transition-all duration-300 shadow-2xl backdrop-blur-md">
            
            {/* Header: Chapter and Metadata */}
            <div className="px-8 py-6 border-b border-white/5 flex flex-col items-start gap-4">
                <div className="flex justify-between w-full items-start">
                    <div className="px-3 py-1 bg-blue-500/10 border border-blue-500/20 rounded-full text-[10px] text-blue-300 tracking-wider font-mono uppercase flex items-center">
                        <BookOpen size={10} className="mr-2" />
                        {mem.chapter}
                    </div>
                    <div className="flex items-center space-x-3 text-xs text-white/30 font-mono">
                        <span className="flex items-center"><Calendar size={12} className="mr-1"/> {mem.date}</span>
                    </div>
                </div>
            ...
            <div className="p-8 flex-1 flex flex-col overflow-y-auto custom-scrollbar">
                <h3 className="text-2xl font-serif italic text-white mb-4 leading-tight group-hover:text-blue-200 transition-colors">{mem.title}</h3>
                
                <p className="text-sm text-white/60 line-clamp-3 leading-relaxed font-light mb-6 italic">
                    "{mem.content}"
                </p>
